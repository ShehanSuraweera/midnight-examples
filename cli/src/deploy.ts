    ...
    wallet.state().pipe(
      Rx.tap((state) => {
        if (state.syncProgress) {
          console.log(
            `Sync progress: synced=${state.syncProgress.synced}`
          );
        }
      }),
...
async function main() {
  ...
  try {
    ...
    // Check if WALLET_SEED exists in environment variables
    if (process.env.WALLET_SEED) {
      ...
      if (walletSeed.length !== 64) {
        console.error("❌ Invalid WALLET_SEED in .env file. Must be 64 characters.");
        process.exit(1);
      }
    } else {
      // No environment variable found, prompt user
      ...
      if (choice.toLowerCase() === "y" || choice.toLowerCase() === "yes") {
        ...
        // Validate seed length
        if (walletSeed.length !== 64) {
          console.error("❌ Invalid seed. Must be 64 characters.");
          process.exit(1);
        }
      } else {
        ...
        console.log(`📝 Add it to your .env file: WALLET_SEED=${walletSeed}\n`);
      }
    }
